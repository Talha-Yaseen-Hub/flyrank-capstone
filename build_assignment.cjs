...
function cleanWorkspace() {
  const keep = ['.git', '.gitignore', 'LICENSE', 'README.md', '.assignment', 'CLAUDE.md', 'build_assignment.cjs', 'node_modules'];
...
// Clean up old build_assignment.js if it exists
const oldJsFile = path.join(workspaceDir, 'build_assignment.js');
if (fs.existsSync(oldJsFile)) {
  fs.unlinkSync(oldJsFile);
  console.log('Removed obsolete build_assignment.js file.');
}

// 1. Ensure Git remote is tracked
try {
